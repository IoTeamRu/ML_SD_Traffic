...
def train(config: ConfigBox, device: torch.DeviceObjType) -> None:
    ...
    env = SumoEnvironment(
        net_file=os.path.join(
            config.data.source_path, config.train.scenario, env_cfg.net_file
        ),
        route_file=os.path.join(
            config.data.source_path, config.train.scenario, env_cfg.route_file
        ),
    ...
    learner = Learner(env, alg_cfg, device)
    ...
    for step in range(alg_cfg.steps):
        ...
        if not step % 2000:
            print(f"Mean reward after {step} sptes: {np.mean(mean_rew):.3f}")
        ...
        if step == alg_cfg.steps - 1:
            ...
            test_reward = evaluate(test_env, learner, 10, step)
            learner.save(os.path.join(config.train.model_path, config.train.model_file))

    end = time.time() - start
    print(
        f"Training is completed after {alg_cfg.steps} steps with mean reward {test_reward:.3f}. It took {end/60:.2f} mins."
    )


def evaluate(
    env, learner: Callable, evaluate_times: int, step: int, verbose: bool = True
) -> float:
    evaluate_rewards = []
    learner.model.eval()
...
if __name__ == "__main__":
    ...
    print(f'ENV variables: {os.environ["SUMO_HOME"]}, {os.environ["PYTHONPATH"]}')
    config = load_config(config_path)
    print(f"CUDA is available {torch.cuda.is_available()}")
    device = torch.device(
        f"cuda:{int(config.train.device_id)}" if torch.cuda.is_available() else "cpu"
    )
